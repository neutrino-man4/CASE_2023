...
Parameters = recordtype('Parameters','run_n, qcd_train_sample_id, qcd_test_sample_id, sig_sample_id, strategy_id, epochs, read_n')
params = Parameters(run_n=50000, 
                    qcd_train_sample_id='qcdSigQRTrainReco', 
                    qcd_test_sample_id='qcdSigQRTestReco',
        ...
        if do_qr:
            ...
            for quantile in quantiles:
                ...
                if train_qr:
                    ...
                    lambda_discriminator = qrwf.train_LBSQR(quantile, mixed_train_sample, mixed_valid_sample, params)
                    discriminator_path = qrwf.save_QR(lambda_discriminator, params, experiment, quantile, xsec)
                    model_paths.append(discriminator_path)

                else: # else load discriminators
                    lambda_discriminator = qrwf.load_QR(params, experiment, quantile, xsec, model_path_date)
            ...
            for q, model_path in zip(quantiles, model_paths):
                discriminator = disc.L4QRDiscriminator_KerasAPI(quantile=q, loss_strategy=lost.loss_strategy_dict[params.strategy_id])
            ...
            plot_name = 'lambdaBS_iter_%s_multi_discr_cut_'%iteration+params.sig_sample_id.replace('Reco','')+'_x'+str(int(xsec))
