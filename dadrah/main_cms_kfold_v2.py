    ...
    for k in range(params.kfold):
        ...
        for sig_sample_id, sig_in_training_nums, mass in zip(signals, sig_in_training_nums_arr, masses):
            ...
            for xsec, sig_in_training_num in zip(xsecs, sig_in_training_nums):
                ...
                # train QR model
                discriminator = qrwf.train_L4QR(quantile, mixed_train_sample, mixed_valid_sample, params)

                discriminators.update({inv_quantile_str(quantile)+"_%s"%str(k):discriminator})
...
for k in range(0,params.kfold):    
    for quantile in quantiles:
        ...
        for l in range(0,params.kfold):
            ...
            qrcuts = discriminators[inv_quantile_str(quantile)+"_%s"%str(l)].predict(bin_centers)
            coeffs = np.polyfit(bin_centers, qrcuts, params.poly_order)
            polynomial = np.poly1d(coeffs) 
            ...
            #chunks[k] = qrwf.predict_QR_fold(discriminators[inv_quantile_str(quantile)+"_%s"%str(l)], chunks[k], inv_quant, counter)
            counter += 1
