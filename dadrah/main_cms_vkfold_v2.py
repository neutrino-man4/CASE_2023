...
mss = np.random.exponential(scale=500, size=100)
mss = mss+1450
#mss = np.linspace(1450,5300,400)
...
for k in range(0,params.kfold):    
    ...
    for q,quantile in enumerate(quantiles):
        inv_quant = round((1.-quantile),2)
        qrcuts = np.empty([0, len(mss)])
        counter = 0 
        for l in range(0,params.kfold):
            if k == l:
                continue

            yss = discriminators["fold_%s"%str(l)].predict(mss)
            print("XXXXXXXXXXXXXXXXXX")
            print("XXXXXXXXXXXXXXXXXX")
            print("XXXXXXXXXXXXXXXXXX")
            print(np.array(mss).shape)
            print(np.array(yss).shape)
            
            split_yss = np.array(np.split(np.array(yss),len(mss)),dtype=object)[:,q]
            qrcuts = np.append(qrcuts, split_yss[np.newaxis,:], axis=0) 

        x_all = np.ravel(mss + np.zeros_like(qrcuts))
