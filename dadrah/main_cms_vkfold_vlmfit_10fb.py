...
# signals
resonance = 'na'
signals = ['XToYYprimeTo4Q_MX3000_MY400_MYprime170_narrowReco']#,'WkkToWRadionToWWW_M3000_Mr170Reco']
each_10fb_are = ['XToYYprimeTo4Q_MX3000_MY400_MYprime170_narrowReco':171]
masses = [3000]
xsecs = [0.]
#sig_in_training_nums_arr = signal_contamin[(resonance, xsecs[0])] # TODO: adapt to multiple xsecs
sig_injection = 50 #50fb
sig_in_training_nums_arr = [int(sig_injection/10)*each_10fb_are[s] for s in signals]

# quantiles
quantiles = [0.05, 0.3, 0.5, 0.7, 0.9, 0.95]
#quantiles = [0.1, 0.3]
regions = ["A","B","C","D","E"]
#quantiles = [0.5]
...
for k in range(0,params.kfold):
    for q,quantile in enumerate(quantiles):
        ...
        print("#####")
        print(f"quantile {int(inv_quant*100)}")
        print(f"fold {k}")
        print("#####")
        ...
        df.to_csv('models_lmfit_csv_10fb/bkg_lmfit_modelresult_fold_{}_quant_q{:02}.csv'.format(str(k),int(inv_quant*100)))
...
for q,quantile in enumerate(quantiles):
    ...
    data = {'par':pars,'err':parserr}
    df = pd.DataFrame.from_dict(data)
    df.to_csv('models_lmfit_csv_10fb/sig_lmfit_modelresult_quant_q{:02}.csv'.format(int(inv_quant*100)))
