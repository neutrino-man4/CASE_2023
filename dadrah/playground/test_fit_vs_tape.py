...
def make_model(n_layers, n_nodes, initializer='he_uniform', activation='relu'):
...
qcd_sig_sample = js.JetSample.from_input_dir('qcdSigReco', paths.sample_dir_path('qcdSigReco'), read_n=int(1e6))
print('training on {} events'.format(len(qcd_sig_sample)))
x_train = qcd_sig_sample['mJJ'] #normalize_data(qcd_sig_sample['mJJ'])
...
# *** build model *** #
model = make_model(n_layers=n_layers, n_nodes=n_nodes, initializer=initializer, activation=activation)
model.summary()

# *** fit model *** #
if fit_type == 'keras':
    model.compile(loss=loss_fn, optimizer=optimizer, run_eagerly=True) # Adam(lr=1e-3) TODO: add learning rate
    model.fit(x_train, y_train, epochs=epochs, batch_size=batch_sz, shuffle=False, verbose=2)
else:
...
plot_cut(model, qcd_sig_sample, plot_name='qr_cut_'+fit_type+'_fit_relu')
