...
#               runtime params
# ********************************************************
def check_if_exists(signal_name,base_dir,return_list=False):
    mass_sf_indices=[1,3,5,7,8,9,10,11]
    tags=['JES_up','JES_down','JER_up','JER_down','JMS_up','JMS_down','JMR_up','JMR_down','nominal']
    exists_ok=[]
    for tag in tags:
        out_filepath=os.path.join(base_dir,tag,signal_name+'.h5')
...
test_samples_DATA_ORIG = ['qcdSigTest']
test_samples_QStar = [
                'QstarToQW_M_2000_mW_170',
                'QstarToQW_M_2000_mW_25',
                'QstarToQW_M_2000_mW_400',
...
new_dir = '/storage/9/abal/CASE/new_signals'
#test_samples = test_samples_MC_ORIG + test_samples_X_TO_YY + test_samples_Qstar_RS_W 
#test_samples = ['WkkToWRadionToWWW_M3000_Mr170']

test_samples=sorted(os.listdir(new_dir))
test_samples = test_samples_QStar
...
for sample_id in test_samples:
    ...
    if check_if_exists(sample_id,result_paths.sample_dir_path(sample_id + 'Reco')):
        print(f"All signal files for {sample_id} exist. Skipping")
        continue
    
    check_existence=check_if_exists(sample_id,result_paths.sample_dir_path(sample_id + 'Reco'),return_list=True)
    # ********************************************
    #               read test data (events)
    # ********************************************
    for ik,key in enumerate(JE_tags):
        
        if check_existence[ik]: 
            print(f'File exists for {key}. Skipping')
            continue
        
        try:
            list_ds = tf.data.Dataset.list_files(os.path.join(input_paths.sample_dir_path(sample_id),key)+'/*')
            print(list_ds)
        except Exception as e:
            print('Something went wrong. File is probably not present. Printing directory contents and moving on.')
            for f in os.listdir(os.path.join(input_paths.sample_dir_path(sample_id),key)):
                print(f)
                with open(os.path.join(result_paths.sample_dir_path(reco_sample.name),'error_log.csv'), 'a+') as f:
                    writer=csv.writer(f)
                    data=[sample_id,key,e]
        ...
        for file_path in list_ds.take(n_testsamples):
            ...
            try:
                ...
                print('Something went wrong. File is probably not present.')
                for f in os.listdir(os.path.join(input_paths.sample_dir_path(sample_id),key)):
                    print(f)
                with open(os.path.join(result_paths.sample_dir_path(reco_sample.name),'error_log.csv'), 'a+') as f:
                    writer=csv.writer(f)
            ...
            result_dir=os.path.join(result_paths.sample_dir_path(reco_sample.name, mkdir=True),key)
            pathlib.Path(result_dir).mkdir(parents=True,exist_ok=True)
            s_name=sdr.path_dict['sample_names'][reco_sample.name]
            print(f'writing results for {s_name} to {os.path.join(result_dir,file_name)}')
            reco_sample.dump_with_orig(os.path.join(result_dir, file_name))
            print('#####################################')
            print('*************************************')
